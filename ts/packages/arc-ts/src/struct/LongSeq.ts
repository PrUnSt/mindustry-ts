// 源: arc-core/src/arc/struct/LongSeq.java
// 迁移说明: 可扩容、有序/无序的 long 数组. items 用 number[] (float64 表示 long, 整数精度 ±2^53 内). 无序时 remove 避免内存拷贝.
import {Mathf} from './Mathf';

/** 可扩容、有序或无序的 long 数组. */
export class LongSeq{
    items: number[];
    size = 0;
    ordered: boolean;
    ...
    constructor(a?: any, b?: any, c?: any, d?: any){
        ...
        }else if(a instanceof LongSeq){
            const array = a as LongSeq;
    ...
    /** @see LongSeq(number[]) */
    static with(...array: number[]): LongSeq{
    ...
    addAll(array: LongSeq): void;
    addAll(array: LongSeq, offset: number, length: number): void;
    addAll(...array: number[]): void;
    addAll(array: number[], offset: number, length: number): void;
    addAll(...args: any[]): void{
        if(args.length === 1){
            const a = args[0];
            if(a instanceof LongSeq){
     ...
     * @return 本数组是否被修改.
     */
    removeAll(array: LongSeq): boolean{
        let size = this.size;
    ...
    hashCode(): number{
        ...
        let h = 1;
        for(let i = 0, n = this.size; i < n; i++)
            h = (Math.imul(h, 31) + (items[i] ^ (items[i] >>> 32))) | 0;
        return h;
    }
